...
public:
  ...
  bool Intersects(const LifeState &pat) const {return !AreDisjoint(pat);}
  bool Intersects(const LifeState &pat, int targetDx, int targetDy) const {return !AreDisjoint(pat, targetDx, targetDy);}
  ...
  LifeState Moved(const int &x, const int &y) const {
    LifeState output = *this;
    output.Move(x, y);
    return output;
  }
  ...
  uint64_t inline Rokicki2Neighbors(
    ...
    return  (bB1 ^ bU1 ^ ts1 ^ s1) & //bit 1 of neighbor count
            ...
            ~(ts0 ^ s0); //bit 0 of neighbor count
  }
